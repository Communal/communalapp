...
const CreateCommunityFormContext =
  createContext<CreateCommunityFormContextInterface>({
...
export default function CreateCommunity() {
  const [flow, setFlow] = useState<
    'community-details' | 'image-upload' | 'invite'
  >('community-details');
  ...
  return (
    <Box className="flex flex-row items-center justify-center h-screen">
      <CreateCommunityFormContext.Provider
        value={{ newCommunityData, setNewCommunityData }}>
        <Card className="w-[600px] max-md:w-[80%]">
          <Heading size="lg">
            {flow === 'community-details' && 'Creating a new community'}
            {flow == 'image-upload' &&
              'Upload a profile image for your community'}
            {flow === 'invite' && 'Invite friends & members'}
          </Heading>
          <Subtitle size="sm">
            {flow === 'community-details' &&
              'You cannot the community name later. Only community Title and Description are editable.'}
            {flow === 'image-upload' &&
              'Recommended image size is between 200x200 to 1000x1000'}
            {flow === 'invite' && ''}
          </Subtitle>
          ...
          <Flex alignItems="center" justifyContent="center" className="my-12">
            {flow === 'community-details' && (
              <Button
                ...
                disabled={
                  !newCommunityData.title || !newCommunityData.communityName
                }>
                Save & Upload Logo
              </Button>
            )}
            {flow === 'image-upload' && (
              <>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setFlow('community-details')}>
...
function CommunityDetailsTab() {
  const { newCommunityData, setNewCommunityData } = useContext(
    CreateCommunityFormContext,
  );
  return (
    <motion.div
      initial={{
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  strech
                  className="justify-start truncate hover:scale-100">
              ...
              <DropdownMenuContent
                ...
                <DropdownMenuLabel>
                  {communityCategories.length + ' Categories'}
                </DropdownMenuLabel>
    ...
    <motion.div
      ...
      <Box className="my-12">
        <Flex direction="column" alignItems="center" justifyContent="center">
          <Avatar
            fallback={newCommunityData.title[0]}
            size="lg"
            image={newCommunityData.imageURL}
          />
